...
function plot_mcmc_results(model::SegmentedModel, chain; n_plot_points=200, legend=:best)
    ...
    # Create the plot
    p = scatter(model.x, model.y, label="Data", alpha=0.5, legend=legend)
...
plot(p_sine,dpi=600,size = (600,400),titlefontsize=12,legendfontsize=8,titlelocation=:left)
println("Saving sine wave comparison plot...")
savefig(p_sine, "img/sine_map_vs_mcmc5.png")
